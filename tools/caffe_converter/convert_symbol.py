...
def _parse_proto(prototxt_fname):
    ...
    for i, layer in enumerate(layers):
        ...
        if layer.type == 'Eltwise':
            type_string = 'mx.symbol.broadcast_plus'
            param_string = ""
            need_flatten[name] = False
        ...
        if layer.type == 'AbsVal':
            ...
            need_flatten[name] = need_flatten[mapping[layer.bottom[0]]]
        if layer.type == 'Slice':
            type_string = 'mx.symbol.SliceChannel'
            need_flatten[name] = False
            for tp in layer.top:
                need_flatten[tp] = True
            param_string  = 'num_outputs=%d'%(len(layer.top))
        ...
        if skip_layer:
            ...
            if len(bottom) == 1:
                ...
                if type_string == 'mx.symbol.SliceChannel':
                    for ti in range(len(layer.top)):
                        symbol_string += "%s = %s[%d]\n"%(layer.top[ti], name, ti) 
            else:
        ...
        if type_string == 'mx.symbol.SliceChannel':
            for j in range(len(layer.top)):
                mapping[layer.top[j]] = layer.top[j]
            output_name = layer.top
        else:
            for j in range(len(layer.top)):
