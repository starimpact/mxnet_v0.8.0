...
//    std::unique_ptr<dmlc::Stream> fi(dmlc::Stream::Create(fname, "r"));
    dmlc::MemoryFixedSizeStream fi((void*)param_bytes, param_size);
    mxnet::NDArray::Load(&fi, &data, &names);
  }
...
int MXKVStoreInitPartial(KVStoreHandle handle,
  ...
  for (mx_uint i = 0; i < num; ++i) {
    ...
    v_shapes[i] = TShape(ori_shapes + i * 2, ori_shapes + i * 2 + 2);
    v_indexes[i] = Intlist(ori_indexes + offset, ori_indexes + offset + dim0);
    offset += dim0;
  }
